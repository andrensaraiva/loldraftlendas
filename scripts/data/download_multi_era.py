...
def get(item):
    ...
    direct=year==2024
    return str(year),dict(url=url,sha256=digest,bytes=path.stat().st_size,publisher="Oracle's Elixir public Google Drive" if direct else "Oracle's Elixir; third-party public mirror",originalPublisher='https://lol.timsevenhuysen.com/matchdata/',accessedAt='2026-09-10' if year==2025 else '2026-09-09' if direct else '2026-09-07',note="The publisher's public Drive file (ID 1v6LRphp2kYciU4SXp0PCjEMuev1bDejc; advertised as 79,169,638 bytes) was quota-limited. This 79,130,187-byte mirror is hash-pinned but not independently authenticated against that later Drive revision. Raw CSV ignored; normalized snapshots committed." if year==2025 else 'Public source is hash-pinned; normalized snapshots are committed and raw CSVs are ignored.' if direct else 'Mirrors preserve attributed data but are not independently authenticated by the publisher. Raw CSVs ignored; normalized snapshots committed.')
