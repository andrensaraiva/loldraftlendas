...
O [pipeline de retratos](docs/player-portrait-pipeline.md) possui um piloto aprovado de dez ilustrações artísticas geradas para o projeto. Elas não são fotografias, materiais oficiais nem endosso dos jogadores. A interface exibe esses retratos, usa as silhuetas derivadas se um asset falhar e mantém o avatar CSS neutro para identidades ainda fora do catálogo.
