...
- Home e instruções.
- Cinco escolhas imediatas: TOP → JUNGLE → MID → ADC → SUPPORT. Cada posição sorteia ano e região dentre grupos disponíveis.
- 390 versões pesquisadas: Worlds 2015, 2017, 2019, 2020, 2022 e 2023; LCK, LPL, Europa e NA. Cada posição mostra exatamente três candidatos do ano/região sorteados.
...
- Escolha com um clique, confirmação visual de 320 ms e avanço automático. Layout compacto, estatísticas em detalhes e explicação progressiva do cálculo.
...
- Progressão do torneio, histórico de séries, todas as condições de resultado e novo draft.
...
Cinco slots chineses de 2015 usam evidência sazonal complementar e confiança de desempenho zero, sem inventar métricas ausentes. O relatório documenta essas exceções. A calibração não elimina a vantagem regional observada da LCK/LPL.
...
Um único temporizador cancelável controla a reprodução na interface. A partida é sorteada uma vez; mudar velocidade, modo ou pausar não a sorteia novamente. O resultado só entra no placar da série ao terminar sua apresentação. Relatórios são mantidos no histórico em memória até começar outro draft ou atualizar a página.

A partida fica apenas na memória e reinicia ao atualizar a página. Nenhuma chave, login, perfil ou persistência de usuário foi adicionado.
...
```sh
npm run data:multi:build     # Recria a produção a partir de snapshots offline
...
npm run test:e2e  # Fluxo completo em desktop e mobile
npm run build    # TypeScript e bundle de produção
```

Os testes Playwright usam Edge instalado. Em outra máquina, troque `channel: 'msedge'` em `playwright.config.ts` pelo navegador disponível ou instale o Chromium do Playwright. Há testes para campanha campeã, derrota no Suíço, reinício, imagens, ausência de rolagem horizontal, desempates, BO3 e encerramento antecipado de BO5.
