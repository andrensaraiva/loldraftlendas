...
- Onboarding versionado em cinco etapas na primeira visita, com pulo, teclado, swipe, movimento reduzido e reabertura por **Como jogar**.
...
O [pipeline de retratos](docs/player-portrait-pipeline.md) possui um piloto aprovado de dez ilustrações artísticas e um primeiro lote de expansão com outras dez identidades aguardando revisão visual. Elas não são fotografias, materiais oficiais nem endosso dos jogadores. A interface exibe os retratos aprovados, usa silhuetas derivadas para itens pendentes ou em caso de falha e mantém o avatar CSS neutro para identidades ainda fora do catálogo.
