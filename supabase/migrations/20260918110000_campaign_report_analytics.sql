...
add constraint analytics_events_event_name_check check (event_name in (
  ...
  'play_again', 'save_resumed', 'campaign_paused', 'campaign_abandoned', 'how_to_play_opened',
...
rename to record_analytics_event_before_campaign_report;

revoke all on function public.record_analytics_event_before_campaign_report(uuid, uuid, uuid, text, text, jsonb) from public, anon, authenticated;
...
as $$
begin
  if p_event_name <> 'campaign_report_opened' then
    perform public.record_analytics_event_before_campaign_report(
  ...
  if exists (
    ...
    where property_key not in ('game_mode', 'game_plan')
  ) then
    raise exception 'analytics property is not allowed';
  end if;
  if not (p_properties ? 'game_mode') or p_properties ->> 'game_mode' not in ('classic', 'almanac') then
