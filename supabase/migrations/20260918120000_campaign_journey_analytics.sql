...
add constraint analytics_events_event_name_check check (event_name in (
  ...
  'campaign_paused', 'campaign_abandoned', 'how_to_play_opened', 'rating_details_opened',
  'share_started', 'share_completed', 'card_downloaded', 'challenge_opened',
  'challenge_started', 'challenge_completed', 'challenge_link_copied',
  'daily_opened', 'daily_started', 'daily_completed'
));

alter function public.record_analytics_event(uuid, uuid, uuid, text, text, jsonb)
rename to record_analytics_event_before_campaign_journey;

revoke all on function public.record_analytics_event_before_campaign_journey(uuid, uuid, uuid, text, text, jsonb) from public, anon, authenticated;
...
as $$
begin
  if p_event_name not in ('journey_node_opened', 'journey_downloaded') then
    perform public.record_analytics_event_before_campaign_journey(
  ...
  if p_properties is null or jsonb_typeof(p_properties) <> 'object' or octet_length(p_properties::text) > 300 then
    raise exception 'invalid analytics properties';
  end if;
  if p_event_name = 'journey_downloaded' and p_properties <> '{}'::jsonb then
    raise exception 'journey download properties are not allowed';
  end if;
  if p_event_name = 'journey_node_opened' then
    if exists (
      select 1 from jsonb_object_keys(p_properties) property_key
      where property_key not in ('stage', 'series_index')
    ) then
      raise exception 'analytics property is not allowed';
    end if;
    if p_properties ->> 'stage' not in ('swiss', 'quarters', 'semis', 'final') then
      raise exception 'invalid journey stage';
    end if;
    if p_properties ->> 'series_index' !~ '^[0-9]{1,2}$' then
      raise exception 'invalid journey series index';
    end if;
  end if;
