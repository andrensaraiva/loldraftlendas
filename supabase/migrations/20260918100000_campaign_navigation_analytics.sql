...
rename to record_analytics_event_before_campaign_navigation;

revoke all on function public.record_analytics_event_before_campaign_navigation(uuid, uuid, uuid, text, text, jsonb) from public, anon, authenticated;
...
as $$
begin
  if p_event_name not in ('campaign_paused', 'campaign_abandoned') then
    perform public.record_analytics_event_before_campaign_navigation(
      p_client_event_id,
      p_campaign_id,
      p_session_id,
      p_event_name,
  ...
  if exists (
    ...
    where property_key not in ('screen')
  ) then
    raise exception 'analytics property is not allowed';
  end if;
  if not (p_properties ? 'screen') or p_properties ->> 'screen' not in (
    'draft', 'team', 'tournament', 'match', 'result'
  ) then
    raise exception 'invalid campaign screen';
  end if;
